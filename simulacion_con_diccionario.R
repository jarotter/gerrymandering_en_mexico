...
detect_conflicting_lim_scope <- function(E, V, u, v, dict){
  n <- nrow(E)
  indices <- rep(FALSE, n)
  ...
  for(i in 1:n){
    ...
    if(x %in% c(u,v) | y %in% c(u,v)){
      indices[i] <- is_conflicting(V, x, y, dict)
    }else{
      indices[i] <- E[i, 'conflictivos']
    }
  }
  return(indices)
}
...
una_iteracion <- function(G, E, V, beta, wd, wp, wi, dict){
  ...
  if(revisar_conexidad(E, V_temp)){
    new_conflicting <- detect_conflicting_lim_scope(E, V_temp, u, v, dict)
